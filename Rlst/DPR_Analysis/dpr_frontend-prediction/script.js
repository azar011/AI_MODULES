...
// Generate beautiful curved line chart with AI forecast predictions & confidence intervals
function createForecastLineChart(id, histLabels, histData, foreLabels, foreData, foreLower, foreUpper, title, colorTheme = 'indigo') {
    const theme = THEME_COLORS[colorTheme] || THEME_COLORS.indigo;
    const combinedLabels = [...histLabels, ...foreLabels];
    
    // Hist dataset has nulls for forecast points
    const histDatasetData = [...histData, ...Array(foreData.length).fill(null)];
    
    // Forecast dataset starts at the last historical value to connect the line smoothly
    const foreDatasetData = [...Array(histData.length - 1).fill(null), histData[histData.length - 1], ...foreData];
    
    // Upper/Lower bounds datasets
    const upperDatasetData = [...Array(histData.length - 1).fill(null), histData[histData.length - 1], ...foreUpper];
    const lowerDatasetData = [...Array(histData.length - 1).fill(null), histData[histData.length - 1], ...foreLower];
        ...
        document.getElementById(id),
        {
            ...
            data: {
                ...
                datasets: [
                    ...
                    {
                        label: `${title} (AI Forecast)`,
                        data: foreDatasetData,
                        borderColor: '#c084fc', // premium purple shade
                    ...
                    legend: { 
                        ...
                        labels: {
                            filter: function(item) {
                                // Hide confidence bounds from legend for cleaner display
...
loadBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        let mlData = null;
        try {
            const mlResponse = await fetch(`${BASE_URL}/predict/${type}/${name}`);
            if (mlResponse.ok) {
                mlData = await mlResponse.json();
            }
        } catch (mlErr) {
            console.error("Error fetching ML predictions:", mlErr);
        }

        chartsContainer.style.display = "grid";

        // Update Dynamic Titles in the DOM for Specific Dashboards
        document.getElementById("title-chart1").textContent = "7-Day Completion Trend + 7d AI Forecast";
        ...
        let aiBannerHtml = "";
        if (mlData) {
            const riskClass = mlData.risk_level.toLowerCase();
            const tomorrowPred = mlData.tomorrow_prediction;
            const anomalyAlert = mlData.is_anomaly 
                ? `<span class="risk-badge high" style="font-size: 8px; margin-left: 6px; padding: 2px 6px; vertical-align: middle;">⚠️ Anomaly Alert</span>` 
                : "";
            
            aiBannerHtml = `
                <!-- AI Predictions Horizontal Banner -->
                <div class="kpi-card ai-pred-card" style="display: flex; flex-direction: row; justify-content: space-between; align-items: center; padding: 16px 24px; background: linear-gradient(135deg, #f5f3ff 0%, #edd8ff 100%); border: 1.5px solid #c084fc !important; margin-bottom: 20px; flex-wrap: wrap; gap: 16px;">
                    <div style="display: flex; align-items: center; gap: 16px;">
                        <span style="font-size: 28px;">🧠</span>
                        <div>
                            <h4 style="font-size: 15px; font-weight: 800; color: #6b21a8; margin: 0; display: flex; align-items: center; gap: 8px;">
                                AI Operational Forecast (Tomorrow) ${anomalyAlert}
                            </h4>
                            <p style="font-size: 11px; color: #7e22ce; margin: 4px 0 0 0; font-weight: 500;">
                                Model Accuracy &bull; R²: ${mlData.metrics.r2} | MAE: ${mlData.metrics.mae}%
                            </p>
                        </div>
                    </div>
                    <div style="display: flex; gap: 28px; align-items: center; flex-wrap: wrap;">
                        <div style="display: flex; flex-direction: column; gap: 2px;">
                            <span style="font-size: 9px; font-weight: 700; color: #701a75; text-transform: uppercase; letter-spacing: 0.5px;">Predicted Rate</span>
                            <span style="font-size: 20px; font-weight: 800; color: #6b21a8; line-height: 1;">${tomorrowPred}%</span>
                        </div>
                        <div style="display: flex; flex-direction: column; gap: 2px;">
                            <span style="font-size: 9px; font-weight: 700; color: #701a75; text-transform: uppercase; letter-spacing: 0.5px;">Lapse Risk</span>
                            <div style="line-height: 1; margin-top: 2px;">
                                <span class="risk-badge ${riskClass}" style="margin: 0; padding: 3px 8px; font-size: 10px;">${mlData.risk_level} Risk</span>
                            </div>
                        </div>
                        <div style="display: flex; flex-direction: column; gap: 2px;">
                            <span style="font-size: 9px; font-weight: 700; color: #701a75; text-transform: uppercase; letter-spacing: 0.5px;">Trend Status</span>
                            <span style="font-size: 14px; font-weight: 700; color: #6b21a8; line-height: 1; margin-top: 4px;">${mlData.trend}</span>
                        </div>
                        <div style="display: flex; flex-direction: column; gap: 2px;">
                            <span style="font-size: 9px; font-weight: 700; color: #701a75; text-transform: uppercase; letter-spacing: 0.5px;">Confidence</span>
                            <span style="font-size: 14px; font-weight: 700; color: #6b21a8; line-height: 1; margin-top: 4px;">${mlData.confidence_score}%</span>
                        </div>
                    </div>
                </div>
            `;
        ...
        kpiContainer.innerHTML = `
            ...
            <div class="kpi-grid">
                ...
                <div class="kpi-card comparison-card-v2">
                    <div class="comparison-header">
                        <span class="comparison-card-title">Submit Comparison</span>
        ...
        // CHART 1: 7 Day Trend line + Forecast
        if (mlData && mlData.predictions && mlData.predictions.length > 0) {
            charts.push(
                createForecastLineChart(
                    "chart1",
                    data.trend_7_days.map(x => formatDate(x.report_date)),
                    data.trend_7_days.map(x => x.completion_percentage),
                    mlData.predictions.map(x => formatDate(x.report_date)),
                    mlData.predictions.map(x => x.predicted_completion_percentage),
                    mlData.predictions.map(x => x.confidence_lower),
                    mlData.predictions.map(x => x.confidence_upper),
                    "Completion %",
                    "indigo"
                )
            );
